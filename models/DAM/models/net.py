...
class Net(object):
    ...
    def __init__(self, conf):
        ...
        if self._conf['word_emb_init'] is not None:
            ...
            # self._word_embedding_init = pickle.load(open(self._conf['word_emb_init'], 'rb'))  # Ally: change to adapt to python3
            # Ally: change to below lines
            with open(self._conf['word_emb_init'], 'rb') as f:
                self._word_embedding_init = pickle.load(f, encoding="bytes")
            # Ally: end of change
        else:
    ...
    def build_graph(self):
        with self._graph.as_default():
            ...
            if self._word_embedding_init is not None:
                word_embedding_initializer = tf.compat.v1.constant_initializer(self._word_embedding_init)
            else:
            ...
            self.tt_turns_len = tf.compat.v1.placeholder(
                tf.int32,
                shape=[self._conf["batch_size"]])

            self.every_turn_len = tf.compat.v1.placeholder(
                tf.int32,
                shape=[self._conf["batch_size"], self._conf["max_turn_num"]])
    
            self.response = tf.compat.v1.placeholder(
                tf.int32, 
                shape=[self._conf["batch_size"], self._conf["max_turn_len"]])

            self.response_len = tf.compat.v1.placeholder(
                tf.int32, 
                shape=[self._conf["batch_size"]])
            ...
            #define operations
            #response part
            Hr = tf.nn.embedding_lookup(params=self._word_embedding, ids=self.response)
            ...
            for turn_t, t_turn_length in zip(list_turn_t, list_turn_length):
                Hu = tf.nn.embedding_lookup(params=self._word_embedding, ids=turn_t) #[batch, max_turn_len, emb_size]
            ...
            with tf.compat.v1.variable_scope('loss'):
                ...
                self.saver = tf.compat.v1.train.Saver(max_to_keep = self._conf["max_to_keep"])
